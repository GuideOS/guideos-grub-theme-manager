...
# =======================================================================
# Titel: GuideOS Grub Theme Selector
# Version: 1.3
# Autor: Nightworker
# Datum: 2025-12-25
...
class GrubThemeSelector(Gtk.Window):
    def __init__(self):
        super().__init__(title="GuideOS GRUB Theme Selector")
    ...
    def setup_ui(self):
        ...
        # --- RECHTE SEITE (Vorschau & Controls - Ohne Rahmen) ---
    ...
    def on_apply(self, btn):
        ...
        cmd = f"cp {GRUB_FILE} {GRUB_FILE}.backup && sed -i 's|^#\\?GRUB_THEME=.*|GRUB_THEME=\"{theme_path}\"|' {GRUB_FILE} && update-grub"
        try:
            subprocess.run(["pkexec", "bash", "-c", cmd], check=True)
            self.update_status("Erfolgreich angewendet!")
        except:
            self.update_status("Fehler oder Abbruch.")
